<template>
  <div class="baidu-map" :class="className" :style="{height: height}">
    <div class="map-container h-100" ref="mapContainer"></div>
...
<script>
  import { BMap, BMAP_ANCHOR_BOTTOM_RIGHT, queue } from 'utils/BaiduMap'
  ...
  export default {
    ...
    mounted () {
      queue.exec(() => {
        const map = this.map = new BMap.Map(this.$refs.mapContainer, this.options)
        const center = typeof this.center === 'string' ? this.center : new BMap.Point(this.center.lng, this.center.lat)
