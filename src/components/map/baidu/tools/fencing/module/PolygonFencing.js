...
import { BMap, BMAP_DRAWING_POLYGON } from '../../../../../../utils/BaiduMap'
...
export default class PolygonFencing extends Fencing {
  ...
  renderPolygon (points, fencingStyle) {
    ...
    const polygon = new BMap.Polygon(points, fencingStyle)
    this.map.addOverlay(polygon)
    return polygon
  }

  render (points, options) {
    const overlay = this.renderPolygon(points, _.get(options, 'fencingStyle'))
  ...
  getDetail () {
    ...
    const area = geoUtils.getPolygonArea(points)
    return {
      type: this.type,
      area,
      points
