<template>
  <ul class="pagination" :class="classes" v-show="isVisible">
    <li class="page-item" v-if="prevText" :class="{disabled: isPrevDisabled}">
      <a class="page-link" href="#" @click.prevent="prev" v-html="prevText"></a>
    </li>
    <template v-for="item in list">
      <li class="page-item" v-if="item.type == ITEM_TYPE.PAGE" :class="{active: index == item.index}">
        <a class="page-link" href="#" @click.prevent="select(item.index)" v-text="item.index+1"></a>
      </li>
      <li class="page-item page-item-ellipsis" v-else-if="item.type == ITEM_TYPE.ELLIPSIS">
        <a
          class="page-link"
          href="#"
          @click.prevent="enterPage(item)"
          v-show="!item.open">
          <span v-html="ellipseText"></span>
        </a>
        <input
          type="number"
          :min="1"
          :max="pages"
          :ref="item.name"
          v-model="item.value"
          v-show="item.open"
          @blur="inputBlur(item)"
          class="form-control"
          :class="[size ? `form-control-${size}` : '']"
          @keyup.enter="inputEnter(item)" style="width: 60px;">
      </li>
    </template>
    <li class="page-item" v-if="nextText" :class="{disabled: isNextDisabled}">
      <a class="page-link" href="#" @click.prevent="next" v-html="nextText"></a>
    </li>
    <li class="page-item" v-if="showTotalCount">
      <span class="page-link">共{{totalCount}}个</span>
    </li>
  </ul>
</template>

<script>
  import _ from 'lodash'
  import { delay } from 'helper.js'

  const ITEM_TYPE = {
    PAGE: 0,
    ELLIPSIS: 1
  }

  export default {
    name: 'Pagination',
    props: {
      totalCount: Number,
      size: {
        type: String,
        default: 'sm'
      },
      align: {
        type: String,
        default: 'center'
      },
      pageSize: {
        type: Number,
        default: 10
      },
      pageNo: {
        type: Number,
        default: 1
      },
      prevText: {
        type: String,
        default: '上一页'
      },
      nextText: {
        type: String,
        default: '下一页'
      },
      edges: {
        type: Number,
        default: 1
      },
      // min 3
      displayedPages: {
        type: Number,
        default: 5
      },
      useStartEdge: {
        type: Boolean,
        default: true
      },
      useEndEdge: {
        type: Boolean,
        default: true
      },
      ellipseText: {
        type: String,
        default: '&hellip;'
      },
      showTotalCount: {
        type: Boolean,
        default: true
      },
      // 当总数小于每页数量时是否隐藏
      lessSizeThenHide: {
        type: Boolean,
        default: true
      }
    },
    watch: {
      pageNo () {
        if (this.pageNo !== 0 && this.index !== this.pageNo) {
          this.index = this.pageNo - 1
        }
      }
    },
    computed: {
      classes () {
        return [
          this.size ? `pagination-${this.size}` : '',
          this.align ? `justify-content-${this.align}` : ''
        ]
      },
      ITEM_TYPE () {
        return ITEM_TYPE
      },
      isPrevDisabled () {
        return this.index === 0
      },
      isNextDisabled () {
        return this.index === this.pages - 1
      },
      interval () {
        return {
          start: Math.ceil(this.index > this.halfDisplayed ? Math.max(Math.min(this.index - this.halfDisplayed, (this.pages - this.displayedPages)), 0) : 0),
          end: Math.ceil(this.index > this.halfDisplayed ? Math.min(this.index + this.halfDisplayed, this.pages) : Math.min(this.displayedPages, this.pages))
        }
      },
      halfDisplayed () {
        return this.displayedPages / 2
      },
      pages () {
        return Math.ceil(this.totalCount / this.pageSize)
      },
      list () {
        let results = []
        const pages = this.pages
        const edges = this.edges
        const { start, end } = this.interval

        // start edges
        if (start > 0 && edges > 0) {
          if (this.useStartEdge) {
            const end = Math.min(edges, start)
            results = results.concat(_.range(0, end).map(d => {
              return {
                type: ITEM_TYPE.PAGE,
                index: d
              }
            }))
          }
          if (edges < start && (start - edges !== 1)) {
            results.push({
              open: false,
              name: 'before',
              value: results[results.length - 1].index + 2,
              index: results[results.length - 1].index + 2,
              type: ITEM_TYPE.ELLIPSIS
            })
          } else if (start - edges === 1) {
            results.push(edges)
          }
        }

        // interval link
        results = results.concat(_.range(start, end).map(d => {
          return {
            type: ITEM_TYPE.PAGE,
            index: d
          }
        }))

        // end edges
        if (end < pages && edges > 0) {
          if (pages - edges > end && (pages - edges - end !== 1)) {
            results.push({
              open: false,
              name: 'end',
              value: results[results.length - 1].index + 2,
              index: results[results.length - 1].index + 2,
              type: ITEM_TYPE.ELLIPSIS
            })
          } else if (pages - edges - end === 1) {
            results.push({
              type: ITEM_TYPE.PAGE,
              index: end
            })
          }
          if (this.useEndEdge) {
            const begin = Math.max(pages - edges, end)
            results = results.concat(_.range(begin, pages).map(d => {
              return {
                type: ITEM_TYPE.PAGE,
                index: d
              }
            }))
          }
        }

        this.items = results

        return results
      },
      isVisible () {
        return this.totalCount > 0 && (!this.lessSizeThenHide || this.totalCount > this.pageSize)
      }
    },
    data () {
      return {
        // 从1开始
        index: this.pageNo - 1,
        items: []
      }
    },
    methods: {
      setCurrent (n) {
        this.index = n - 1
      },
      select (index) {
        if (index !== this.index) {
          this.index = index
          this.$emit('select', index + 1)
        }
      },
      prev () {
        this.select(this.index - 1)
      },
      next () {
        this.select(this.index + 1)
      },
      enterPage (item) {
        item.open = true
        delay(() => {
          this.$refs[item.name][0].focus()
        })
      },
      inputBlur (item) {
        if (item.value !== item.index) {
          this.inputEnter(item)
        } else {
          item.open = false
        }
      },
      inputEnter (item) {
        item.open = false
        this.select(item.value - 1)
      }
    }
  }
</script>
