...
const name = 'Response'
const old = root[name]
const initContentKey = 'init' + name
const win = window
const doc = document
const docElem = doc.documentElement
const ready = $.domReady || $
const $win = $(win)
const DMS = typeof DOMStringMap !== 'undefined'
const AP = Array.prototype
const OP = Object.prototype
const push = AP.push
const concat = AP.concat
const toString = OP.toString
const owns = OP.hasOwnProperty
const isArray = Array.isArray || function (item) {
...
const screenMax = screenW > screenH ? screenW : screenH
const screenMin = screenW + screenH - screenMax
const deviceW = function () { return screenW }
const deviceH = function () { return screenH }
const regexFunkyPunc = /[^a-z0-9_\-\.]/gi
const regexTrimPunc = /^[\W\s]+|[\W\s]+$|/g
const regexCamels = /([a-z])([A-Z])/g
const regexDashB4 = /-(.)/g
const regexDataPrefix = /^data-(.+)$/

const procreate = Object.create || function (parent) {
      /** @constructor */
  function Type () {}
...
const namespaceIt = function (eventName, customNamespace) {
...
const media = matchMedia ? bind(matchMedia, win) : function () {
  return {}
}
...
const band = bind(between, viewportW)
const wave = bind(between, viewportH)
const device = {
...
function map (stack, fn, scope) {
  const r = []
  for (let l = stack.length, i = 0; i < l;) r[i] = fn.call(scope, stack[i], i++, stack)
  return r
}
...
function each (stack, fn, scope) {
  ...
  for (let l = stack.length, i = 0; i < l;) fn.call(scope || stack[i], stack[i], i++, stack)
  return stack
}
...
function affix (stack, prefix, suffix) {
  ...
  for (let l = stack.length, i = 0; i < l; i++) { stack[i] == null || r.push(prefix + stack[i] + suffix) }
  return r
}
...
function merge (r, s) {
  ...
  else for (const k in s) owns.call(s, k) && void 0 !== s[k] && (r[k] = s[k])
  return r
}
...
function dpr (decimal) {
  ...
  if (decimal == null) return dPR || (dpr(2) ? 2 : dpr(1.5) ? 1.5 : dpr(1) ? 1 : 0) // approx
 ...
 */
function parse (s) {
  let n // undefined, or becomes number
  return typeof s !== 'string' || !s ? s
    : s === 'false' ? false
      : s === 'true' ? true
        : s === 'null' ? null
          : s === 'undefined' || (n = (+s)) || n === 0 || s === 'NaN' ? n
            : s
}
...
function eachAttr (el, fn, exp) {
  ...
  const test = typeof exp === 'boolean' ? /^data-/ : undefined
  for (i = 0, l = el.attributes.length; i < l;) {
    const a = el.attributes[i++]
    if (a) {
      n = '' + a.name
      test && test.test(n) !== exp || a.value === null || fn.call(el, a.value, n, a)
    }
  }
}
...
function getDataset (el) {
  ...
  let ob = DMS && el.dataset
  if (ob) return ob // native
...
function setViaObject (el, ob, fn) {
  for (const n in ob) owns.call(ob, n) && fn(el, n, ob[n])
}
...
function dataset (el, k, v) {
  ...
  const exact = isArray(k) && datatize(k[0])
  if (typeof k === 'object' && !exact) {
    ...
    if (void 0 === v) {
      ...
      return k == null ? v : exact ? parse(k) : '' + k // normalize
    }
...
function sel (keys) {
  ...
  const r = []
  for (let k, i = 0, l = keys.length; i < l;) { (k = keys[i++]) && r.push('[' + datatize(k.replace(regexTrimPunc, '').replace('.', '\\.')) + ']') }
  return r.join()
}
...
function inX (elem, verge) {
  const r = rectangle(first(elem), verge)
...
function detectMode (elem) {
  ...
  const srcElems = { img: 1, input: 1, source: 3, embed: 3, track: 3, iframe: 5, audio: 5, video: 5, script: 5 }
  const modeID = srcElems[elem.nodeName.toLowerCase()] || -1
  ...
  return modeID < 4 ? modeID : elem.getAttribute('src') !== null ? 5 : -5
}
...
function store ($elems, key, source) {
  let valToStore
  if (!$elems || key == null) throw new TypeError('@store')
  source = typeof source === 'string' && source
  route($elems, function (el) {
    if (source) valToStore = el.getAttribute(source)
    else if (detectMode(el) > 0) valToStore = el.getAttribute('src')
    else valToStore = el.innerHTML
    valToStore == null ? deletes(el, key) : dataset(el, key, valToStore)
  })
  return Response
}
...
const Elemset = (function () {
  const crossover = event.crossover
    //, update = event.update
  const min = Math.min
  ...
  return {
    ...
    reset: function () {
      const subjects = this.breakpoints
    ...
    configure: function (options) {
      ...
      this.uid = suid++
      if (this.verge === null) this.verge = min(screenMax, 500)
      if (!(this.fn = propTests[prop])) throw new TypeError('@create')

      // If we get to here then we know the prop is one one our supported props:
      // 'width', 'height', 'device-width', 'device-height', 'device-pixel-ratio'
      if (this.dynamic === null) this.dynamic = prop.slice(0, 6) !== 'device'

      this.custom = isCustom[prop]
      const prefix = this.prefix ? sift(map(compact(this.prefix), sanitize)) : ['min-' + prop + '-']
      const aliases = prefix.length > 1 ? prefix.slice(1) : 0
      ...
      if (isArray(points)) {
        each(points, function (v) {
          if (!v && v !== 0) throw new Error('invalid breakpoint')
...
function crossover (prop, fn) {
  let temp
  const eventCrossover = event.crossover
  if (typeof prop === 'function') { // support args in reverse
    temp = fn
    fn = prop
    prop = temp
  }
  const eventToFire = prop ? ('' + prop + eventCrossover) : eventCrossover
...
function create (args) {
  route(args, function (options) {
    ...
    const elemset = procreate(Elemset).configure(options)
    const verge = elemset.verge
    const breakpoints = elemset.breakpoints
    const scrollName = namespaceIt('scroll')
    const resizeName = namespaceIt('resize')

    if (!breakpoints.length) return

    // Identify the lowest nonzero breakpoint. (They're already sorted low to high by now.)
    const lowestNonZeroBP = breakpoints[0] || breakpoints[1] || false

    ready(function () {
      const allLoaded = event.allLoaded
      const lazy = !!elemset.lazy
...
// Initialize
ready(function () {
  let settings = dataset(doc.body, 'responsejs')
  const parse = win.JSON && JSON.parse || $.parseJSON
